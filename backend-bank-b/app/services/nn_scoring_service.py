...
MODEL_PATH = settings.GLOBAL_MODEL_PATH
...
SCORE_CENTER = settings.SCORE_CENTER
LOG_ODDS_FACTOR = settings.LOG_ODDS_FACTOR
...
class NeuralNetworkScoringService:
    ...
    def predict_score(self, features: Dict) -> Tuple[int, str, float]:
        ...
        try:
            ...
            # This provides better score distribution across the configured range
            import math
            ...
            # Scale to credit score range using config values
            alt_score = int(round(SCORE_CENTER + LOG_ODDS_FACTOR * log_odds))

            # Clamp to valid range from config
            alt_score = max(SCORE_MIN, min(SCORE_MAX, alt_score))
