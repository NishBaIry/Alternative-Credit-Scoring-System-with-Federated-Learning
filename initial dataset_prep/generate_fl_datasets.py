...
INPUT_FILE = "data/credit_scoring_dataset.csv"
...
def apply_domain_constraints(df):
    ...
    # UPI spending patterns based on default risk
    # Good borrowers (default_flag=0): 40-60% of monthly income
    # Bad borrowers (default_flag=1): 85-110% of monthly income (overspending)
    if 'default_flag' in df.columns and 'upi_txn_amt_avg' in df.columns:
        for idx in df.index:
            monthly_inc = df.loc[idx, 'monthly_income']
            if pd.isna(monthly_inc) or monthly_inc <= 0:
                monthly_inc = 30000  # Default fallback
            
            is_defaulter = df.loc[idx, 'default_flag'] == 1
            
            if is_defaulter:
                # Bad borrowers: overspend (85-110% of income)
                spend_ratio = np.random.uniform(0.85, 1.10)
            else:
                # Good borrowers: conservative spending (40-60% of income)
                spend_ratio = np.random.uniform(0.40, 0.60)
            
            df.loc[idx, 'upi_txn_amt_avg'] = monthly_inc * spend_ratio
...
def generate_synthetic_data(real_data, num_synthetic, bank_name):
    ...
    # Apply initial domain constraints (without UPI adjustment)
    print(f"    Applying initial domain constraints...")
    # Temporarily skip UPI adjustment by applying constraints without default_flag
    synthetic_df_temp = synthetic_df.drop('default_flag', axis=1, errors='ignore')
    ...
    # NOW apply UPI spending patterns based on final default_flag
    print(f"    Adjusting UPI spending based on default risk...")
    for idx in synthetic_df.index:
        monthly_inc = synthetic_df.loc[idx, 'monthly_income']
        if pd.isna(monthly_inc) or monthly_inc <= 0:
            monthly_inc = 30000  # Default fallback
        
        is_defaulter = synthetic_df.loc[idx, 'default_flag'] == 1
        ...
        synthetic_df.loc[idx, 'upi_txn_amt_avg'] = monthly_inc * spend_ratio
...
# Adjust UPI spending for real data based on default_flag
print(f"  • Adjusting UPI spending patterns for real data...")
for idx in df_bank_a_real.index:
    monthly_inc = df_bank_a_real.loc[idx, 'monthly_income']
    if pd.isna(monthly_inc) or monthly_inc <= 0:
        monthly_inc = 30000
    
    is_defaulter = df_bank_a_real.loc[idx, 'default_flag'] == 1
    ...
    df_bank_a_real.loc[idx, 'upi_txn_amt_avg'] = monthly_inc * spend_ratio
...
# Save Bank A dataset with proper decimal precision (5 decimal places)
df_bank_a.to_csv(BANK_A_OUTPUT, index=False, float_format='%.5f')
...
# Adjust UPI spending for real data based on default_flag
print(f"  • Adjusting UPI spending patterns for real data...")
for idx in df_bank_b_real.index:
    monthly_inc = df_bank_b_real.loc[idx, 'monthly_income']
    if pd.isna(monthly_inc) or monthly_inc <= 0:
        monthly_inc = 30000
    
    is_defaulter = df_bank_b_real.loc[idx, 'default_flag'] == 1
    ...
    df_bank_b_real.loc[idx, 'upi_txn_amt_avg'] = monthly_inc * spend_ratio
...
# Save Bank B dataset with proper decimal precision (5 decimal places)
df_bank_b.to_csv(BANK_B_OUTPUT, index=False, float_format='%.5f')
